...
<script setup lang="ts">
  ...
  const breadcrumbItems = computed<BreadcrumbItem[]>(() => {
    ...
    // 展示完整父级菜单链（如：系统管理 / 角色管理）。
    // 不再因一级菜单（isFirstLevel）而折叠为仅当前页，否则“系统管理”等父级菜单会丢失。
    // 过滤掉无标题的包裹/容器路由，避免产生空的面包屑项。
    let items = matched.map(createBreadcrumbItem).filter((item) => item.meta?.title)
