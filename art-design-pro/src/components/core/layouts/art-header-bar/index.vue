    ...
    >
      <div class="flex-c flex-1 min-w-0 leading-15" style="display: flex">
        ...
        <div class="flex-c c-p" @click="toHome" v-if="isTopMenu">
          <img v-if="site.logo" :src="site.logo" class="logo-img pl-4.5" alt="logo" />
          <ArtLogo v-else class="pl-4.5" />
          <p v-if="width >= 1400" class="my-0 mx-2 ml-2 text-lg">
            {{ site.title || AppConfig.systemInfo.name }}
          </p>
        </div>
      ...
      <div class="flex-c gap-2.5">
        ...
        <ArtIconButton
          ...
          :class="{ 'has-new-notice': noticeStore.unreadCount > 0 }"
          @click="visibleNotice"
        >
          <div
        ...
        <ArtIconButton
          ...
          :class="{ 'has-new-chat': chatUnreadCount > 0 }"
          @click="openChat"
        >
          <div
            v-if="chatUnreadCount > 0"
            class="absolute top-1 right-1 min-w-3.5 h-3.5 px-1 !bg-danger rounded-full text-[10px] leading-3.5 text-white text-center"
          >
...
<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    // 初始化时拉取真实未读消息，使铃铛角标准确
    noticeStore.loadMessages()
    // 监听聊天未读数变化
    mittBus.on('chatUnreadChange', (count: number) => {
      chatUnreadCount.value = count || 0
    })
  })

  onUnmounted(() => {
    document.removeEventListener('click', bodyCloseNotice)
    mittBus.off('chatUnreadChange')
  })
  ...
  const { goHome, refresh } = useCommon()
...
<style lang="scss" scoped>
  ...
  /* 有新消息时铃铛持续摇晃 */
  .notice-button.has-new-notice :deep(.art-svg-icon) {
    animation: shake 0.6s ease-in-out infinite;
    animation-delay: 2s;
  }

  .notice-button.has-new-notice:hover :deep(.art-svg-icon) {
    animation: shake 0.5s ease-in-out;
  }
  ...
  /* 有新消息时聊天图标持续摇晃 */
  .chat-button.has-new-chat :deep(.art-svg-icon) {
    animation: shake 0.6s ease-in-out infinite;
    animation-delay: 2s;
  }

  .chat-button.has-new-chat:hover :deep(.art-svg-icon) {
    animation: shake 0.5s ease-in-out;
  }
