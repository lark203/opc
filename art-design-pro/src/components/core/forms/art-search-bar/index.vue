...
<script setup lang="ts">
  ...
  const cloneModelValue = (value: any) => {
    if (!value) return {}
  ...
  initialModelValue.value = cloneModelValue(modelValue.value as any)
  ...
  const getFieldValue = (key: string) => (modelValue.value as any)?.[key]

  const setFieldValue = (key: string, value: unknown) => {
    const normalizedValue = normalizeFieldValue(value)

    if (normalizedValue === undefined) {
      delete (modelValue.value as any)?.[key]
      return
    }

    ;(modelValue.value as any)[key] = normalizedValue
  }
  ...
  const getSanitizedOutput = () => {
    return (sanitizeOutputValue(cloneModelValue(modelValue.value as any)) || {}) as Record<
      string,
      any
    >
  }
  ...
  const handleReset = () => {
    ...
    Object.keys(modelValue.value as any).forEach((key) => {
      delete (modelValue.value as any)[key]
    })
    Object.assign(modelValue.value as any, cloneModelValue(initialModelValue.value))
