  ...
  <div class="cutter-container">
    <div class="cutter-component">
      ...
      >
        ...
        <template #confirm>
          <ElButton type="primary" style="margin-left: 10px">裁剪</ElButton>
        </template>
      </ImgCutter>
    </div>
...
<script setup lang="ts">
  ...
  // 记录最近一次裁剪结果，避免裁剪结果经 v-model 回写后误触发重新加载（导致视觉放大）
  const lastCroppedUrl = ref('')

  // 计算属性：整合所有ImgCutter的props
  // 注意：刻意排除 imgUrl，避免裁剪结果回写后 vue-img-cutter 将其作为新源图重新加载（导致视觉放大）
  ...
  watch(
    () => props.imgUrl,
    (newVal) => {
      // 裁剪结果回写到 imgUrl 时不重新加载（否则会将小图拉伸填满裁切框，造成视觉放大）
      if (newVal && newVal === lastCroppedUrl.value) {
        return
      }
      if (newVal) {
        lastCroppedUrl.value = ''
  ...
  function cutDownImg(result: CutterResult) {
    lastCroppedUrl.value = result.dataURL
    // 预览框展示裁剪结果（主裁切区保持原图，不重新加载裁剪结果）
    temImgPath.value = result.dataURL
