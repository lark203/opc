        ...
        <div>
          <div>
            <div class="flex items-start mb-3 last:mb-0">
              ...
              <span>{{ currentUser.user?.nickName || currentUser.user?.userName || '未登录' }}</span>
            </div>
            <div class="flex items-start mb-3 last:mb-0">
            ...
            <div class="flex items-start mb-3 last:mb-0">
              ...
              <div class="flex flex-wrap gap-2">
                <ElTag v-for="button in ((currentUser as any).permissions || (currentUser as any).buttons || [])" :key="button" size="small" type="info">
                  {{ button }}
                </ElTag>
                <span v-if="!((currentUser as any).permissions?.length || (currentUser as any).buttons?.length)" class="italic text-g-500">无权限码</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <ElCard class="art-card-xs">
        ...
        <div class="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-4">
          <div
           ...
           :class="{
              'bg-theme/12 !border-theme': (currentUser.user?.nickName || currentUser.user?.userName) === account.userName
            }"
          >
            <div class="mb-4">
              <div>
            ...
            <div class="text-right">
              <ElButton
                v-if="(currentUser.user?.nickName || currentUser.user?.userName) !== account.userName"
                type="primary"
