...
import { dashboardFallbackRoutes } from '../modules/dashboard'
...
export class MenuProcessor {
  ...
  private async processBackendMenu(): Promise<AppRouteRecord[]> {
    ...
    const convertedList = this.convertBackendMenu(list)

    const mergedList = [...convertedList]

    const existingDashboard = mergedList.find((item) => item.path === dashboardFallbackRoutes.path)
    if (!existingDashboard) {
      mergedList.unshift(dashboardFallbackRoutes)
    }

    return this.filterEmptyMenus(mergedList)
  }

  /**
   * 将后端返回的菜单数据转换为前端格式
   * - 将 hidden 字段转换为 meta.isHide
   * - 将 meta.noCache 转换为 meta.keepAlive 的反向值
   * - 将 meta.activeMenu 转换为 meta.activePath
   * - 将后端的 Layout 组件转换为前端的 /index/index
   * - 确保组件路径以 / 开头
   */
  private convertBackendMenu(menuList: any[]): AppRouteRecord[] {
    ...
    for (const item of menuList) {
      ...
      if (component === 'Layout') {
        ...
        component = `/${component}`
      } else if (!component && item.children && item.children.length > 0) {
        component = '/ParentView/index'
      }

      const isWrappedMenu =
        item.path === '/' && !item.meta && item.children && item.children.length === 1
      ...
      if (isWrappedMenu) {
        ...
        const convertedChild: AppRouteRecord = {
          ...child,
          component: childComponent,
          meta: {
            ...childMeta,
            // 注意：isMenuFrame/内链等被包裹菜单的 hidden 标记在「父级」路由上，
            // 子级（child）并未设置 hidden。必须继承父级 hidden，否则后台设置隐藏无效。
            isHide: item.hidden || child.hidden || false,
            keepAlive: childMeta.noCache !== true,
            activePath: childMeta.activeMenu || undefined,
            icon: childMeta.icon || ''
          }
        }
        if (child.children && child.children.length > 0) {
          convertedChild.children = this.convertBackendMenu(child.children)
        }
        result.push(convertedChild)
      } else {
        const convertedItem: AppRouteRecord = {
          ...item,
          component,
          meta: {
            ...meta,
        ...
        if (item.children && item.children.length > 0) {
          convertedItem.children = this.convertBackendMenu(item.children)
        }
  ...
  private filterEmptyMenus(menuList: AppRouteRecord[]): AppRouteRecord[] {
    return menuList
      ...
      .filter((item) => {
        ...
        if (item.meta && item.meta.title && item.path && item.path.trim()) {
          return true
        }
