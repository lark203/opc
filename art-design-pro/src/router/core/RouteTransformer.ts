...
export class RouteTransformer {
      ...
      converted.children = [
        {
          ...route,
          component:
            component && !this.shouldSkipComponent(component)
  ...
  /**
   * 需要跳过的组件路径（这些功能已通过弹窗实现）
   */
  private skipComponents: string[] = [
   ...
   * 判断是否需要跳过该组件
   */
  private shouldSkipComponent(component: string): boolean {
    return this.skipComponents.includes(component)
  }
  ...
  private handleNormalRoute(converted: ConvertedRoute, component: string | undefined): void {
    if (component && !this.shouldSkipComponent(component)) {
