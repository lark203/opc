...
public class SysOssServiceImpl implements ISysOssService, OssService {
    ...
    /**
     * 预览OSS对象，以 inline 方式内联返回文件字节，供浏览器直接展示（如图片预览）。
     * <p>
     * 通过后端代理从对象存储读取，浏览器无需直连对象存储地址，
    ...
    public ResponseEntity<byte[]> preview(Long ossId) {
        ...
        return OssFactory.instance(sysOss.getService())
            .download(sysOss.getFileName(), (result, inputStream) -> {
                // 尝试解析媒体类型，如果解析失败或对象存储未返回具体类型，则使用 application/octet-stream
                MediaType mediaType;
                try {
                    mediaType = MediaType.parseMediaType(result.contentType());
                } catch (Exception e) {
                ...
                // 对象存储未返回具体类型时，依据文件名兜底推断，保证浏览器可内联预览
                if (MediaType.APPLICATION_OCTET_STREAM.equals(mediaType)) {
                    Optional<MediaType> guessed = MediaTypeFactory.getMediaType(sysOss.getOriginalName());
                    if (guessed.isPresent()) {
                        mediaType = guessed.get();
                    }
                }
                String percentEncodedFileName = FileUtils.percentEncode(sysOss.getOriginalName());
                // 使用 inline 而非 attachment，确保浏览器内联渲染而不是触发下载
                return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION,
                        "inline; filename=%s;filename*=utf-8''%s".formatted(percentEncodedFileName, percentEncodedFileName))
                    .contentType(mediaType)
                    .contentLength(result.size())
                    .body(IoUtil.readBytes(inputStream));
            });
    ...
    private SysOssVo matchingUrl(SysOssVo oss) {
        ...
        if (instance.verifyConfig(config -> AccessPolicy.PRIVATE.equals(config.accessControlPolicyConfig().accessPolicy()))) {
            ...
            // 公开桶：基于当前客户端配置实时重建可访问 URL（domain 优先，否则使用 endpoint），
            // 避免浏览器沿用上传时存储的旧地址（如内网 endpoint）而无法打开
            oss.setUrl(instance.getUrl(oss.getFileName()));
        }
        return oss;
    }
}
