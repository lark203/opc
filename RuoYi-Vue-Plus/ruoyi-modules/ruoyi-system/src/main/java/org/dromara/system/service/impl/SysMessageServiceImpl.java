...
public class SysMessageServiceImpl implements ISysMessageService, MessageService {
    ...
    @Override
    public void markReadBatch(Long userId, List<Long> messageIds) {
        if (CollUtil.isEmpty(messageIds)) {
            return;
        }
        messageMapper.lambda()
            .in(SysMessage::getMessageId, messageIds)
            .and(wrapper ->
                wrapper.eq(SysMessage::getSendUserIds, GLOBAL_USER_IDS)
                    .or()
                    .findInSet(userId, SysMessage::getSendUserIds)
    ...
    @Override
    public void deleteMessageBatch(Long userId, List<Long> messageIds) {
        if (CollUtil.isEmpty(messageIds)) {
            return;
        }
        messageMapper.lambda()
    ...
    @Override
    public PageResult<SysMessageVo> selectPageMessageList(SysMessageBo notice, PageQuery pageQuery, Long userId) {
        Page<SysMessageVo> page = messageMapper.lambda()
            .eq(StringUtils.isNotBlank(notice.getCategory()), SysMessage::getCategory, notice.getCategory())
            .eq(StringUtils.isNotBlank(notice.getReadStatus()), SysMessage::getReadStatus, notice.getReadStatus())
            .eq(StringUtils.isNotBlank(notice.getType()), SysMessage::getType, notice.getType())
            .eq(StringUtils.isNotBlank(notice.getSource()), SysMessage::getSource, notice.getSource())
            .and(wrapper ->
                wrapper.eq(SysMessage::getSendUserIds, GLOBAL_USER_IDS)
                    .or()
                    .findInSet(userId, SysMessage::getSendUserIds)
