...
public class SysProfileController extends BaseController {
     ...
     * 修改当前登录用户的头像。
     *
     * @param bo 头像参数（包含 ossId）
     * @return 操作结果
     */
    @RepeatSubmit
    @Log(title = "个人信息", businessType = BusinessType.UPDATE)
    @PutMapping("/avatar")
    public R<Void> updateAvatar(@Validated @RequestBody SysUserProfileBo bo) {
        Long avatar = bo.getAvatar();
        if (avatar == null) {
            return R.fail("头像不能为空");
        }
        int rows = DataPermissionHelper.ignore(() -> userService.updateUserAvatar(avatar));
        if (rows > 0) {
            return R.ok();
        }
        return R.fail("修改头像异常，请联系管理员");
    }
    ...
    public R<Void> updatePwd(@Validated @RequestBody SysUserPasswordBo bo) {
        ...
        // 校验新密码是否符合策略
        passwordPolicyService.validatePassword(bo.newPassword(), user.getUserName(), password);
        // 校验密码历史不可重复
        passwordPolicyService.checkPasswordHistory(user.getUserId(), bo.newPassword());
        int rows = DataPermissionHelper.ignore(() -> userService.resetUserPwd(user.getUserId(), BCrypt.hashpw(bo.newPassword())));
        if (rows > 0) {
            TokenBlacklistHelper.kickoutUser(user.getUserId());
            return R.ok();
        }
