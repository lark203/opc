...
public class SysUserImportListener extends AnalysisEventListener<SysUserImportVo> implements ExcelListener<SysUserImportVo> {
    ...
    public SysUserImportListener(Boolean isUpdateSupport) {
        // 初始密码统一从系统配置(sys_option) PASSWORD_INIT 读取，仅用于导入场景；
        // 若未配置则生成一个随机密码，避免使用固定弱口令。
        String initPassword = SpringUtils.getBean(ISysOptionService.class).getOptionValue("PASSWORD", "PASSWORD_INIT");
        if (StrUtil.isBlank(initPassword)) {
            initPassword = RandomUtil.randomString(
                "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz23456789!@#$%", 12);
        }
