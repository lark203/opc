...
public class TemplateEngineUtils {
    ...
    public static List<PathNamedTemplate> getTemplateList(String tplCategory, String dsName, String frontendType) {
        ...
        if (GenConstants.TPL_CRUD.equals(tplCategory)) {
            ...
            // 弹窗子组件模板：仅当 fm/{type}/dialog.*.ftl 存在时生成（vue/react 无此模板则跳过）
            String dialogTemplatePath = findFrontendPageTemplatePath(frontendType, GenConstants.FRONTEND_DIALOG_TEMPLATE_PREFIX);
            if (StringUtils.isNotBlank(dialogTemplatePath)) {
                templates.add(getTemplate(dialogTemplatePath));
            }
    ...
    public static String getFileName(String template, GenTable genTable) {
        ...
        } else if (isFrontendPageTemplate(template, GenConstants.FRONTEND_DIALOG_TEMPLATE_PREFIX)) {
            fileName = StringUtils.format("{}/{}/{}/{}/modules/{}-dialog.{}", frontendPath, frontendPagePath, moduleName,
                businessName, businessName, getFrontendPageExtension(template, GenConstants.FRONTEND_DIALOG_TEMPLATE_PREFIX));
        }
        return fileName;
    }
    ...
    private static String getFrontendPageTemplatePath(String frontendType, String templatePrefix) {
        String path = findFrontendPageTemplatePath(frontendType, templatePrefix);
        if (StringUtils.isBlank(path)) {
            throw new ServiceException(StringUtils.format("未找到前端模板: {}",
                getFrontendPageTemplatePathPattern(resolveFrontendType(frontendType), templatePrefix)));
        }
        return path;
    }

    /**
     * 查找前端页面模板路径，不存在时返回 {@code null}（用于可选模板，如 dialog）。
     *
     * @param frontendType   前端模板类型
     * @param templatePrefix 页面模板文件名前缀
     * @return 页面模板路径，不存在返回 {@code null}
     */
    private static String findFrontendPageTemplatePath(String frontendType, String templatePrefix) {
