...
import org.dromara.common.satoken.utils.TokenBlacklistHelper;
...
public class PasswordAuthStrategy implements IAuthStrategy {
    ...
    public LoginVo login(String body, SysClientVo client) {
        ...
        // 检查密码是否过期，并在登录结果中返回标记
        PasswordExpiryInfo passwordExpiry = passwordPolicyService.getPasswordExpiryInfo(user.getPasswordUpdateTime());
        if (passwordExpiry.expired()) {
            log.warn("用户{}密码已过期，需强制修改", username);
        } else if (passwordExpiry.expiringSoon()) {
            log.warn("用户{}密码将在{}天后过期，请及时修改", username, passwordExpiry.daysLeft());
        }
        ...
        TokenBlacklistHelper.clearKickoutUser(user.getUserId());

        LoginVo loginVo = new LoginVo();
        loginVo.setAccessToken(StpUtil.getTokenValue());
        loginVo.setExpireIn(StpUtil.getTokenTimeout());
        loginVo.setClientId(client.getClientId());
        loginVo.setPasswordExpired(passwordExpiry.expired());
        loginVo.setPasswordExpiringSoon(passwordExpiry.expiringSoon());
        loginVo.setPasswordExpireInDays(passwordExpiry.daysLeft() < 0 ? null : passwordExpiry.daysLeft());
        return loginVo;
    }
